/* Worksheet 8 — Observability (SEED / not yet live).
   ---------------------------------------------------------------------------
   This is a parking file for Module 8. It holds observability tasks that were
   ...
   NOT yet registered in quiz.js and NOT wired to a tool — Module 8 needs its
   own simulator first (see docs/MODULE_NOTES.md: reflected-sunlight illumination
   with the truck-headlights analogy, eclipse/shadow, telescope-FOV streaks,
   radar 1/r⁴ link budget, four observability modes). When that tool exists,
   flesh out the intro/parts/summary and register t8.

   The `d1`/`d2` ids below are kept from their Module-2 origin; renumber freely
   when Module 8 is built out. */
const WORKSHEET = {
  intro: [
    'Module 8 (Observability) — placeholder. This module will cover HOW we actually see and measure space objects: reflected sunlight and satellite illumination (the same physics as Moon phases), eclipse/shadow, radar vs. optical, the telescope field of view, and how a streaked image yields a position and a motion.',
  ],
  parts: [
