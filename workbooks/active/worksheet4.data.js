...
const WORKSHEET = {
  objectives: [
    'What <b>delta-v</b> (written <b>Δv</b>) is — a change in speed that you spend to reshape an orbit — and why a satellite carries only a <b>finite supply</b>.',
  ...
  tutorial: [
    ...
    'To change an orbit on purpose, you fire a thruster. The size of that shove is measured as <b>delta-v</b> — literally a <b>change in speed</b>, in m/s or km/s. It is the true currency of spaceflight: every satellite launches with a fixed tank of propellant, and Δv is how we count what is left. When the Δv budget runs out the mission is over, even if every other system still works.',
  ...
  summary: [
    ...
    'The drag “paradox”, stated carefully because the short version misleads: <b>drag never pushes a satellite along</b>. It removes energy, the orbit shrinks, and circular speed is higher at lower altitude — so the satellite ends up moving <i>faster</i> while continuously losing energy. Friction did not accelerate it; the smaller orbit it fell into demands more speed.',
  ...
  tasks: [
    ...
    { id:'a1', title:'The limits of a TLE (and continuously-thrusting satellites)',
      ...
      quiz:{ q:'What is a fundamental limitation of representing an orbit as a TLE (six fixed elements at one epoch)?',
        opts:['It can only describe circular orbits, so any real ellipse has to be approximated','It captures one unperturbed ellipse, so it shows neither perturbations nor thrust','It has no field for inclination, so the plane’s tilt must be supplied separately','It only works above the GEO belt, where the atmosphere no longer interferes'],
    ...
    { id:'a2', title:'Impulsive vs. continuous thrust',
      ...
      quiz:{ q:'Why is a big chemical-thruster satellite in high orbit easier to predict than an ion-powered Starlink?',
        opts:['Chemical satellites fly high enough that gravity barely bends their path at all','The chemical one coasts between brief burns; the ion one thrusts continuously','Starlinks are physically too small for ground radar to track with precision','Ion thrusters are far more powerful, so the errors they introduce are larger'],
    ...
    { id:'b1', title:'Delta-v and prograde/retrograde burns',
      ...
      quiz:{ q:'You are in a circular orbit and fire a prograde (speed-up) burn. What happens?',
        opts:['The whole orbit expands uniformly outward, staying a slightly larger circle','Your burn point becomes perigee and the opposite side rises — apogee climbs','The orbit shrinks, since adding speed pulls you into a tighter, faster path','Nothing changes until you burn a second time to close the new orbit off'],
    ...
    { id:'b2', title:'Radial vs. tangential: which holds an orbit up?',
      ...
      quiz:{ q:'To efficiently raise a decaying orbit, in which direction should you burn?',
        opts:['Radially outward, straight away from Earth toward the altitude you want','Tangentially, prograde — along the direction you are already travelling','Radially inward, using the rebound to carry you out the far side','Direction makes no difference; only the size of the burn matters at all'],
    ...
    { id:'b3', title:'What if you kill the tangential speed entirely?',
      ...
      quiz:{ q:'From a circular orbit, you make a burn that brings your tangential (sideways) speed to exactly zero. What happens?',
        opts:['You stay at the same altitude, simply hanging there without moving','You fall straight down, radially inward, all the way toward the Earth','You drift slowly outward, since gravity weakens as the orbit relaxes','You enter a new circular orbit, smaller but still perfectly closed'],
    ...
    { id:'b4', title:'The radial-burn paradox: pushing “up” crashes you down',
      ...
      quiz:{ q:'From a circular orbit, you make a large burn pointed radially outward (straight away from Earth). What happens?',
        opts:['You climb smoothly into a higher circular orbit centred on the same point','The orbit goes lopsided and perigee drops sharply — hard enough, below the surface','Nothing happens, because a purely radial burn cannot change an orbit at all','You escape Earth entirely, since the thrust points directly outward from it'],
    ...
    { id:'c0a', title:'Challenge: get from LEO to GEO your own way',
      ...
      quiz:{ q:'After experimenting, which kind of burn moved you toward GEO most efficiently (most orbit-raising per unit of Δv)?',
        opts:['Radial burns, pushing straight out toward the altitude you are aiming for','Burns along your direction of travel — prograde, speeding you up','Burns against your motion, retrograde, letting gravity do the lifting','Direction made no measurable difference to the fuel each attempt cost'],
    ...
    { id:'c2', title:'Which burn sequence actually reaches circular GEO?',
      ...
      quiz:{ q:'Which sequence correctly delivers a circular orbit at GEO?',
        opts:['Sequence A — a single large prograde burn while still down at LEO','Sequence B — raise apogee to GEO, coast, then burn prograde at apogee','Sequence C — a radial-outward burn held until you drift up to the belt','Sequence D — raise apogee to GEO, coast, then burn retrograde at apogee'],
    ...
    { id:'x1', title:'Escape velocity',
      ...
      quiz:{ q:'Roughly what is escape velocity from the surface of the Earth?',
        opts:['About 1 km/s, a little faster than a rifle bullet leaves the barrel','About 11 km/s, which is roughly 25,000 mph','About 100 km/s, comparable to the speed of the solar wind','About 300,000 km/s, the same as the speed of light in vacuum'],
    ...
    { id:'x2', title:'Crossing escape: the orbit opens into a hyperbola',
      ...
      quiz:{ q:'When a satellite’s speed reaches escape velocity, its eccentricity reaches 1.0 and its orbit…',
        opts:['Becomes a tighter circle, since the extra energy pulls the path inward','Opens from a closed ellipse into an unbound path — it leaves and never returns','Stays exactly the same, because eccentricity does not affect the shape','Stops abruptly, with the satellite losing its orbital motion entirely'],
    ...
    { id:'x4', title:'Escaping is far cheaper prograde than radial',
      ...
      quiz:{ q:'From a circular orbit, which takes far less Δv (fuel): escaping with a prograde burn, or a radial-outward burn?',
        opts:['Radial is cheaper, because it points directly along the escape direction','Prograde is much cheaper — it adds to the speed you already have','They cost exactly the same, since escape depends only on total energy','Neither can reach escape from a circular orbit without a second burn'],
    ...
    { id:'x3', title:'Radial infall: cancel the sideways speed',
      ...
      quiz:{ q:'You are far out at 20,000 km and cancel your orbital (sideways) speed to zero. What happens?',
        opts:['You stay parked at 20,000 km, held there by the balance of forces','You fall radially straight down, speeding up the whole way to impact','You spiral slowly outward as the remaining energy redistributes itself','You settle into a circular orbit at 20,000 km, just a slower one'],
    ...
    { id:'d1', title:'Watch an orbit decay and re-enter',
      ...
      quiz:{ q:'How does the starting altitude affect how quickly a LEO orbit decays from drag?',
        opts:['Higher orbits decay faster, because they sweep through more volume per lap','Lower orbits decay far faster, because the air is exponentially denser down low','All altitudes decay at much the same rate once drag is acting at all','Altitude has no effect; decay depends only on the satellite’s own shape'],
    ...
    { id:'d2', title:'The drag paradox: friction speeds you up',
      ...
      quiz:{ q:'As drag pulls a satellite to a lower orbit, its orbital speed…',
        opts:['Decreases, because friction acts directly against the direction of travel','Increases — drag drops it to a lower orbit, where circular speed is higher','Stays the same, since drag removes energy without changing the speed','Falls to zero quickly, once drag has cancelled the orbital motion'],
    ...
    { id:'d3', title:'Disposal: burning up vs. boosting away',
      ...
      quiz:{ q:'Why are LEO satellites disposed of by re-entry, but GEO satellites are boosted to a “graveyard” orbit instead?',
        opts:['GEO satellites are too massive to move once they reach their final slot','From LEO a small burn lets drag finish the job; from GEO deorbiting costs far more','There is no air near GEO to help, and controlled re-entry is not permitted there','GEO satellites carry no propellant reserve, so they cannot be moved at all'],
    ...
    { id:'e1', title:'Radiation pressure and area-to-mass',
      ...
      quiz:{ q:'Two objects at the same high altitude feel the same sunlight. Which one has its orbit perturbed more by radiation pressure?',
        opts:['The heavier, compact one, since more mass means more momentum to redirect','The lighter one, with more illuminated area per unit of mass','Both are perturbed equally, because they receive identical sunlight','Neither — radiation pressure is far too weak to affect an orbit at all'],
    ...
    { id:'e2', title:'HAMR objects and solar sails',
      ...
      quiz:{ q:'Why are HAMR objects (like tumbling foil debris) so hard to predict?',
        opts:['They are effectively invisible to radar, so no tracking data ever exists','Sunlight pushes them hard, and tumbling keeps changing their exposed area','They travel faster than any tracked object, outrunning the prediction window','They are so light that gravity barely acts on them in the usual way'],
    ...
    { id:'f1', title:'Earth’s bulge swivels the orbit plane',
      ...
      quiz:{ q:'What does Earth’s equatorial bulge (the J2 effect) do to a tilted orbit?',
        opts:['It speeds the satellite up slightly on each equator crossing','It slowly swivels the whole orbit plane around Earth’s axis — the node precesses','It rounds the orbit out, pulling eccentricity gradually toward zero','It has no effect at all unless the satellite fires its thrusters'],
    ...
    { id:'f2', title:'Sun-synchronous orbits: a perturbation as a feature',
      ...
      quiz:{ q:'Why might a sun-synchronous orbit (constant local sun time) be advantageous?',
        opts:['It lets the satellite hover over one chosen spot, the way GEO does','Every pass images a place under the same sun angle and shadows','It removes the need for station-keeping propellant altogether','It keeps the satellite in permanent sunlight, so batteries are unnecessary'],
    ...
    { id:'g1', title:'Keeping “custody” of an object',
      ...
      quiz:{ q:'What does it mean to maintain “custody” of a space object?',
        opts:['Physically capturing the object with another spacecraft and holding it','Keeping an unbroken, up-to-date track of which object it is and where','Holding legal ownership of the satellite and its assigned orbital slot','Storing one perfect TLE and reusing it for the life of the mission'],
    ...
    { id:'g2', title:'Why observations must be targeted',
      ...
      quiz:{ q:'Why are space-surveillance observations usually “targeted” rather than just staring at random sky?',
        opts:['Sensors can watch the whole sky at once, so targeting is a formality','A sensor sees a small patch, so the predicted orbit says where to point','Objects stay invisible until you already know their catalogue name','International law requires each observation to be filed against a target'],
  ...
  exam: [
    { q:'What is "delta-v," and why does it limit a mission?',
      opts:['The satellite’s distance from Earth, which shrinks steadily over the mission','A change in speed you spend to maneuver — the supply is finite and never renewed','The delay in radio signals travelling between the satellite and the ground','The angle between the satellite’s orbit plane and the Earth’s equator'],
    ...
    { q:'To efficiently raise a decaying orbit, which way should you burn?',
      opts:['Tangentially, prograde — along the direction you are already moving','Radially outward, straight away from Earth toward higher altitude','Radially inward, using the resulting swing to carry you outward','It makes no difference at all which way the thrust is pointed'],
    ...
    { q:'What are the two steps of a standard orbit-raising (e.g. LEO→GEO) maneuver?',
      opts:['Point at the target and thrust continuously the whole way there','Kill all of your speed, then fall to the new altitude and settle','Burn prograde to raise apogee, then at apogee match the speed there','Burn radially outward once, then coast and let gravity do the rest'],
    ...
    { q:'Roughly what is escape velocity from the surface of the Earth, and what happens when you reach it?',
      opts:['About 1 km/s; the orbit tightens into a smaller, faster circle','About 11 km/s; the closed ellipse opens and the object leaves for good','About 100 km/s; the satellite loses its orbital motion and stops','About 300,000 km/s; the orbit is unchanged by reaching that speed'],
    ...
    { q:'A satellite loses energy to atmospheric drag. What happens to its speed, and why do TLEs go stale?',
      opts:['It slows and eventually stops, and TLEs stay valid until it does','Its speed is unchanged, and TLEs only go stale after a thruster burn','It speeds up — the lower orbit is faster — and perturbations act continuously','It speeds up, but the orbit stays perfectly predictable indefinitely'],
    ...
    { q:'To deorbit a satellite from GEO so it falls to Earth, roughly what does it take — and how does it compare to reaching GEO in the first place?',
      opts:['Two large burns totalling ~3,900 m/s, mirroring the whole trip up','One retrograde burn of ~1,500 m/s at GEO, dropping perigee into the air','No propellant at all, since the orbit decays on its own over time','A radial burn aimed straight down at the Earth beneath the satellite'],
