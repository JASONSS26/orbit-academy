...
const WORKSHEET = {
  ...
  tasks: [
    ...
    { id:'a1', title:'Δv is the currency — and it’s finite',
      ...
      observe:'the mission has a fixed Δv budget (4,400 m/s), and the plan will be rejected if your two burns add up to more than that. Δv is a hard currency, not an afterthought.',
      think:[
    ...
    { id:'a2', title:'Start your logbook',
      ...
      do:'Copy the Δv logbook table from the tutorial (or print this worksheet). Label two rows — "raise apogee to GEO" and "circularize at GEO" — plus a MISSION TOTAL row, and write the budget (4,400 m/s) at the bottom.',
    ...
    { id:'b3', title:'Total the logbook & check the budget',
      teach:[
        'Now add your two entries. Burn 1 (~2,400 m/s) + burn 2 (~1,460 m/s) ≈ <b>3,860 m/s</b>. Your budget is <b>4,400 m/s</b>, so you have ~540 m/s of margin (a ~10% reserve) — enough to fly the plan and still have propellant for trim burns, threading the approach gates, and station-keeping at the end.',
        'This is the payoff of the logbook: <b>before</b> lighting the engine you know the mission is affordable. If your total had come out over 4,400, you’d need a cheaper plan (or accept the mission is impossible with this tank).',
      ],
      predict:'Add your two logged burns. Do you come in under the 4,400 m/s budget? By how much?',
      do:'Sum your logbook. In the planner, confirm "total Δv" reads about 3,860 / 4,400 m/s and the verdict says the plan is on target. Use <b>✨ solve it for me</b> to check your numbers if needed.',
      observe:'the two burns total ~3,860 m/s, just inside the 4,400 m/s budget — so the plan is flyable. The logbook told you this before you ever fired the engine.',
      think:[
      ...
      quiz:{ q:'Your logbook totals ~3,860 m/s against a 4,400 m/s budget. What does that tell you?',
        opts:['The mission is impossible','The mission is affordable, with ~540 m/s of margin to spare',
              'You need a third burn','You have twice the Δv you need'],
        a:1, why:'Correct — under budget with ~540 m/s to spare, so the plan is flyable with a real reserve for trims and the final approach.',
        feedback:['It’s under budget — the mission is affordable.','','Two burns suffice for a GEO transfer.','The margin is ~540 m/s — a healthy reserve, but nowhere near double.'] } },
    ...
    { id:'d2', title:'LOI — brake BACKWARDS or sling right past',
      ...
      do:'Dial <b>Δv₂</b> to ~900 m/s (the planner flies it BACKWARDS automatically), then press <b>▶ GO — run sim</b> and watch the prediction. Nudge Δv₁ (±1–2 m/s), the lead angle, and Δv₂ until the verdict reads <b>✔ LUNAR CAPTURE predicted</b>. Log Δv₂ and your total against the 4,600 m/s budget.',
      ...
      think:[
        ...
        'Where did your total land against 4,600 m/s, and how much margin is left for trims?',
      ],
  ...
  summary: [
    ...
    'A LEO→GEO transfer is a <b>two-burn</b> maneuver: burn 1 raises apogee to GEO (~<b>2,400 m/s</b>); burn 2 circularizes at GEO (~<b>1,460 m/s</b>). Total ≈ <b>3,860 m/s</b>, inside a 4,400 m/s budget.',
